...
mod bomctl_tests {
    ...
    fn gen_target() {
        let mut file = File::create("test/target").unwrap();
    ...
    fn cleanup() {
        fs::remove_file("test/target").unwrap();

        if Path::new("test/target.bak").exists() {
            fs::remove_file("test/target.bak").unwrap();
        }
    }
    ...
    fn should() {
        ...
        let target_processed = results.unwrap()
            .iter()
            .any(|item| item.path == PathBuf::from("test/target"));
